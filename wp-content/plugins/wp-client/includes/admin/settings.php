<?php
 if ( ! defined( 'ABSPATH' ) ) { exit; } $current_tab = ( empty( $_GET['tab'] ) ) ? 'general' : urldecode( $_GET['tab'] ); $current_subtab = !empty( $_GET['subtab'] ) ? urldecode( $_GET['subtab'] ) : ''; ?>
...
<div class="wrap">
    ...
    <?php if ( $this->wpc_flags['easy_mode'] ) { ?>
    <div class="notice notice-warning wpc_notice fade"><p>
        <?php printf( __( 'For more settings, you can disable "Easy Mode" in <a href="%1$s" target="_blank">%2$s</a>', WPC_CLIENT_TEXT_DOMAIN ), admin_url( 'admin.php?page=wpclients_settings' ), __( 'Settings > General', WPC_CLIENT_TEXT_DOMAIN ) ) ?>
    </p></div>
    <?php } ?>
    ...
    <h2 id="tab-headers2" class="nav-tab-wrapper wpc-nav-tab-wrapper">
        <?php
 $tabs = $this->get_tabs_of_settings(); if ( $tabs ) { foreach ( $tabs as $key => $tab ) { $active = ( $current_tab == $key ) ? ' nav-tab-active' : ''; echo '<a class="nav-tab  ' . $active . '" href="' . admin_url( 'admin.php?page=wpclients_settings&tab=' . $key ) . '" >' . $tab['title'] . '</a>'; } } ?>
    </h2>

    <?php
 if ( !empty( $tabs[$current_tab]['subtabs'] ) ) { $subtabs_html = '<div><ul class="subsubsub wpc-nav-tab-subsubsub">'; $default_subtab = array_keys( $tabs[$current_tab]['subtabs'] ); $current_subtab = !empty( $current_subtab ) ? $current_subtab : $default_subtab[0]; foreach ( $tabs[$current_tab]['subtabs'] as $key => $label ) { $active = ( $current_subtab == $key ) ? 'current' : ''; $subtabs_html .= '<a href="' . admin_url( 'admin.php?page=wpclients_settings&tab=' . $current_tab ) . '&subtab=' . $key . '" class="' . $active . '">' . $label . '</a> | '; } $subtabs_html = rtrim( $subtabs_html, ' | ' ); echo $subtabs_html . '</ul></div>'; } ?>
 ...
 switch( $_GET['msg'] ) { case 'u': _e( 'Settings Updated.', WPC_CLIENT_TEXT_DOMAIN ); break; case 'cl_url': _e( 'Login URL used default names of Wordpress. Settings are not updated.', WPC_CLIENT_TEXT_DOMAIN ); break; case 'pu': _e( 'Pages Updated Successfully.', WPC_CLIENT_TEXT_DOMAIN ); break; case 'pc': _e( 'Pages Re-created Successfully', WPC_CLIENT_TEXT_DOMAIN ); break; case 'ps': _e( 'You are skipped auto-install pages - please do it manually.', WPC_CLIENT_TEXT_DOMAIN ); break; case 't': _e( 'Import was successful', WPC_CLIENT_TEXT_DOMAIN ); break; case 'f': _e( 'Invalid *.xml file', WPC_CLIENT_TEXT_DOMAIN ); break; case 'pr_ng': _e( 'Note: The registration will not work until you select "Payment Gateways". Clients will see a message that "Registration temporarily unavailable".', WPC_CLIENT_TEXT_DOMAIN ); break; case 'pr_f': _e( 'Invalid settings', WPC_CLIENT_TEXT_DOMAIN ); break; case 'pr_na': _e( 'Not all registration levels was saved', WPC_CLIENT_TEXT_DOMAIN ); break; case 'nk': _e( 'Public or(and) Privat Key is empty.', WPC_CLIENT_TEXT_DOMAIN ); break; default: echo stripslashes(urldecode($_GET['msg'])); } ?>
    ...
    <div id="wpc_settings-container">
            ...
            <?php
 $file_name = 'settings_' . $current_tab; $file_name .= !empty( $current_subtab ) ? "_{$current_subtab}.php" : '.php'; if ( file_exists( $this->plugin_dir . 'includes/admin/' . $file_name ) ) { include_once( $this->plugin_dir . 'includes/admin/' . $file_name ); } else { do_action( 'wpc_client_settings_tab_' . $current_tab ); if ( !empty( $current_subtab ) ) { do_action( "wpc_client_settings_tab_{$current_tab}_{$current_subtab}" ); } } ?>
