        ...
        <div class="wpc_tab_container_block">
            ...
            <form action="" name="edit_client" id="edit_client" method="post" enctype="multipart/form-data">
                <?php if ( 'edit_client' == $_GET['tab'] ) { ?>
                    ...
                    <?php if ( current_user_can( 'wpc_manager' ) && ! current_user_can( 'administrator' ) ) { $client_managers = $this->cc_get_assign_data_by_assign( 'manager', 'client', $client_data['ID'] ); ?>
                ...
                <div class="fields_wrapper">
                    <table class="form-table" style="margin-top: 0;">
                        ...
                        <?php $client_data['ID'] = isset( $client_data['ID'] ) ? $client_data['ID'] : ''; if( !$this->wpc_flags['easy_mode'] ) { if( 'edit_client' == $_GET['tab'] ) { $custom_fields = $this->get_custom_fields('admin_edit_client', $client_data['ID']); } else { $custom_fields = $this->get_custom_fields( 'admin_add_client' ); } if ( is_array( $custom_fields ) && 0 < count( $custom_fields ) ) { $this->add_custom_fields_scripts(); foreach( $custom_fields as $key=>$value ) { if ( 'hidden' == $value['type'] ) { echo $value['field']; } else { echo '<tr><th>'; echo ( !empty( $value['label'] ) ) ? $value['label'] : ''; echo '</th><td>'; echo ( !empty( $value['field'] ) ) ? $value['field'] : ''; echo ( !empty( $value['description'] ) ) ? '<br /><span class="description">' . $value['description'] . '</span>' : ''; echo '</td></tr>'; } } } } if( 'edit_client' == $_GET['tab'] ) { do_action( 'wpc_client_edit_client_form_html', $client_data['ID'] ); } else { do_action( 'wpc_client_add_client_form_html' ); } ?>
