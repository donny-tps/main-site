...
<div class="wrap">
                        ...
                        <td>
                            ...
                            <div class="ct-field-type-options">
                                ...
                                <p>
                                    ...
                                    <?php if( !( isset( $custom_field['options'] ) && is_array( $custom_field['options'] ) ) ) { $custom_field['options'] = array(''); } foreach ( $custom_field['options'] as $key=>$field_option ) { ?>
                                        <tr>
                                            <td>
                                            ...
                                            <td>
                                                <input type="text" name="custom_field[options][<?php echo( $key ) ?>][label]"
                                                       value="<?php echo isset( $field_option['label'] ) ? esc_attr( $field_option['label'] ) : '' ?>" />
                                            </td>
                                            <td>
                                                <input type="text" name="custom_field[options][<?php echo( $key ) ?>][value]"
                                                       value="<?php echo isset( $field_option['value'] ) ? esc_attr( $field_option['value'] ) : '' ?>" />
                                            </td>
                                            <td>
                                                <a href="javascript:void(0);" class="dashicons ct-field-add-option"></a>
                                                <a href="javascript:void(0);" class="dashicons ct-field-delete-option"></a>
                        ...
                        <tr id="wpc_field_display_screen_options">
                            <th>
                                <label for="display_screen_options">
                                    <?php printf( __( 'Display on %s table', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['client']['p'] ) ?>
                                    <br>
                            ...
                            <td>
                                <label>
                                    ...
                                    <?php printf( __( 'Add this custom field as column to %s table', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['client']['p'] ) ?>
                                </label>
                                <br />
                                <span class="description"><?php printf( __( 'This option will set a default value for screen options of column in the %s table for users. Each user can individually change it on their screen options.', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['client']['p'] ) ?></span>
                            </td>
                        </tr>
                    <?php } ?>
                    <tr class="mask_field">
 ...
 $mask_type = isset( $custom_field['mask_type'] ) ? $custom_field['mask_type'] : ''; ?>
                    ...
                    <tr id="wpc_field_view">
                        ...
                        <td>
                            <?php
 $wpc_roles = array( 'administrator' => __( 'Administrators', WPC_CLIENT_TEXT_DOMAIN ), 'admin' => sprintf( __( 'WPC-%s', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['admin']['p'] ), 'manager' => sprintf( __( 'WPC-%s', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['manager']['p'] ), 'client' => sprintf( __( 'WPC-%s', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['client']['p'] ), 'staff' => sprintf( __( 'WPC-%s', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['staff']['p'] ), ); $all_places = array( 'admin_add_client' => sprintf( __( 'Display on Admin Add %s Form', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['client']['p'] ), 'admin_add_staff' => sprintf( __( 'Display on Admin Add %s Form', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['staff']['p'] ), 'admin_edit_client' => sprintf( __( 'Display on Admin Edit %s Form', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['client']['p'] ), 'admin_edit_staff' => sprintf( __( 'Display on Admin Edit %s Form', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['staff']['p'] ), 'user_add_client' => sprintf( __( 'Display on Registration %s Form', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['client']['p'] ), 'user_add_staff' => sprintf( __( 'Display on Add %s Form', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['staff']['p'] ), 'user_edit_client' => sprintf( __( 'Display on %s Profile Form', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['client']['p'] ), 'user_profile_staff' => sprintf( __( 'Display on %s Profile Form', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['staff']['p'] ), 'user_edit_staff' => sprintf( __( 'Display on Edit %s Form', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['staff']['p'] ), 'user_view_staff' => sprintf( __( 'Display in %s Directory Table', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['staff']['p'] ), 'admin_screen' => sprintf( __( 'Display on %s Page in Screen Options', WPC_CLIENT_TEXT_DOMAIN ), $this->custom_titles['client']['p'] ), ); $all_variants = array( 'view' => __( 'View', WPC_CLIENT_TEXT_DOMAIN ), 'edit' => __( 'Edit', WPC_CLIENT_TEXT_DOMAIN ), 'hide' => __( 'Hide', WPC_CLIENT_TEXT_DOMAIN ), ); ?>

                            <table cellspacing="6" id="wpc_cf_view">
                                <thead>
                                    <tr>
                                        <th></th>
                                        <?php foreach( $wpc_roles as $role => $title ) { echo '<th class="wpc_td_' . $role . '">' . $title . '</th>'; } ?>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php foreach( $all_places as $place => $name_place ) { $variants = $all_variants ; if ( strpos( $place, 'add' ) !== false || 'admin_screen' == $place ) { array_shift($variants); } if ( 'user_view_staff' == $place ) { unset($variants['edit']); } $class = ''; if( strpos( $place, 'client' ) !== false ) { $class .= ' client_row'; } else if( strpos( $place, 'staff' ) !== false ) { $class .= ' staff_row'; } else { $class .= ' client_row'; } ?>
                                        <tr valign="top" class="wpc_tr_<?php echo $place . $class; ?>">
                                            <th scope="row">
                                                <?php echo $name_place ?>
                                            </th>

                                            <?php foreach( $wpc_roles as $role=>$name_role ) { $empty = true; if( 'admin_' == substr( $place, 0, 6 ) && in_array( $role, array( 'administrator', 'admin', 'manager' ) ) || 'user_' == substr( $place, 0, 5 ) && in_array( $role, array( 'client', 'staff' ) ) ) { if( $role == 'staff' ) { if( $place == 'user_profile_staff' ) { $empty = false; } } else if( $role == 'client' ) { if( $place != 'user_profile_staff' ) { $empty = false; } } else { $empty = false; } } echo '<td class="wpc_td_' . $role . '">'; if ( !$empty ) { ?>
                                                    <select name="custom_field[view][<?php echo $place ?>][<?php echo $role ?>]">
                                                    <?php foreach( $variants as $variant=>$name_variant ) { $choosed = ( !empty( $custom_field['view'][ $place ][ $role ] ) && in_array( $custom_field['view'][ $place ][ $role ], array_keys( $variants ) ) ) ? $custom_field['view'][ $place ][ $role ] : 'edit'; $selected = selected( $choosed, $variant, false); echo '<option value="' . $variant . '" ' . $selected . '>' . $name_variant . '</option>'; } ?>
                                                    </select>
                                                <?php } echo '</td>'; } ?>
                                        </tr>
                                    <?php } ?>
