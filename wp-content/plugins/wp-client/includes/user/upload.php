<?php
 if ( ! defined( 'ABSPATH' ) ) { exit; } ob_start(); global $post, $wpdb, $current_user; if ( !ini_get( 'safe_mode' ) ) { @set_time_limit(0); } $wpc_file_sharing = $this->cc_get_settings( 'file_sharing' ); $include_extensions = ( isset( $wpc_file_sharing['include_extensions'] ) && !empty( $wpc_file_sharing['include_extensions'] ) ) ? $wpc_file_sharing['include_extensions'] : ''; if ( !empty( $include_extensions ) ) { $extesions_array = explode( ',', $include_extensions ); $extesions_array = array_map( array( &$this, 'ltrim_file_extension' ), $extesions_array); $include_extensions = implode( ',', $extesions_array ); } $exclude_extensions = ( isset( $wpc_file_sharing['exclude_extensions'] ) && !empty( $wpc_file_sharing['exclude_extensions'] ) ) ? $wpc_file_sharing['exclude_extensions'] : ''; if ( !empty( $exclude_extensions ) ) { $extesions_array = explode( ',', $exclude_extensions ); $extesions_array = array_map( array( &$this, 'ltrim_file_extension' ), $extesions_array); $exclude_extensions = implode( ',', $extesions_array ); } ?>

<div class="wpc_client_upload_form">

<?php
if ( is_multisite() && !is_upload_space_available() ) { echo '<p>' . __( 'Sorry, you have used all of your storage quota.', WPC_CLIENT_TEXT_DOMAIN ) . '</p>'; } else { $uploader_id = rand( 0, 10000 ); if ( isset( $wpc_file_sharing['client_uploader_type'] ) && 'html5' == $wpc_file_sharing['client_uploader_type'] ) { ?>
        ...
        <form enctype="multipart/form-data" method="post">
            ...
            <?php $include_ext = $include_extensions; if ( !empty( $atts['include'] ) ) { $extesions_array = explode( ',', $atts['include'] ); $extesions_array = array_map( array( &$this, 'ltrim_file_extension' ), $extesions_array ); $include_ext = implode( ',', $extesions_array ); } $exclude_ext = $exclude_extensions; if ( !empty( $atts['exclude'] ) ) { $extesions_array = explode( ',', $atts['exclude'] ); $extesions_array = array_map( array( &$this, 'ltrim_file_extension' ), $extesions_array ); $exclude_ext = implode( ',', $extesions_array ); } ?>
            ...
            <?php echo $this->files()->build_uploader_category_selectbox( $client_id, $uploader_id, $atts ); ?>
