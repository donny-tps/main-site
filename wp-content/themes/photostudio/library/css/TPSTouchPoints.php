...
class TouchPoint {
    ...
    public static function loadAssets() {
        ...
        wp_localize_script('tps-touchpoints', 'ajax_scripts_object', array(
            ...
            'autoOptions' => TouchPoint::getTouchpointNames(),
            'redirecturl' => '/event/',
...
//        die();
            ...
            foreach ($notifications as $notification) {
            	    $sendDate = date('Y-m-d H:i:s', strtotime($_POST['schedule-date_' . $notification->id]));
                    $notifyData = array(
                        'send_date' => $sendDate,
                    );
    ...
    public static function logUpdate($id, $kind) {
        global $wpdb;
        $uid = get_current_user_id();
    ...
    public static function getAllBookings($limit = 1000, $offset = 0) {
        global $wpdb;
        if($_GET['uid']) {
            $uid = $_GET['uid'];
        } else {
        ...
        if(! $uid == 'all') {
            $bookings = $wpdb->get_results('SELECT * FROM  `wp3_touchpoint_bookings` WHERE booker_id = ' . get_current_user_id() . ' ORDER BY creation_date DESC LIMIT ' . $limit . ' OFFSET ' . $offset);
        } else {
...
//        $emails[] = $today + (4 * $gap);
    ...
    public static function emailsToSend() {
        ...
        $emailsToSend = $wpdb->get_results('SELECT * FROM wp3_touchpoint_emails WHERE status = "pending" AND send_date < "' . $now->format('Y-m-d H') . '"');
