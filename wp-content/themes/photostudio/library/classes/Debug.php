...
/**
 * Static class for handling various developer debugging functionality
 *
 * @author Ben Rowe
 * @static
 */
class Debug {
    ...
    public static $restrict = array('150.101.114.79');
    ...
    public static function dumpToEmail($to, $data, $subject = 'Debug::dumpToEmail') {
    ...
    /**
     *
     * @return boolean true if the remote request comes from the Clickers IP
     */
    public static function isClickersIP() {

        if ($_SERVER['REMOTE_ADDR'] == '61.69.84.26'  || $_SERVER['REMOTE_ADDR'] == '127.0.0.1' || $_SERVER['REMOTE_ADDR'] == '192.168.0.17') {
            return true;
        }
    ...
    public static function log($msg, $type = 'Clicker.LOG') {
        if (is_array($msg)) {
            $msg = print_r($msg, true);
        }
        $file = file_put_contents(
                BASEDIR . 'log/' . $type, '[' . date('Y-m-d H:i:s') . '] ' . $msg . "\r\nfrom: " . self::logFrom() . "\r\n", FILE_APPEND
        );
    }
